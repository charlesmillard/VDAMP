...
%    The FISTA, SISTA or SUREIT algorithm with subband-wise wieghing for
...
%         hist: if saveHist == 1, history object formed with attributes
%                r1: (nx*ny*maxIter) image representation of vector subject to thresholding
%                var_est: (maxIter) estimate of MSE of r1
...
% Copyright (C) 2019  Siemens Healthineers
    ...
    if saveHist
        hist.r1 = zeros(nx,ny,60);
        hist.x = zeros(nx,ny,maxIter);
        hist.var_est = zeros(1, maxIter);
    ...
    for iter = 1:maxIter
        ...
        % thresholding
        if sureFlag
            C_thr_new = multiscaleSUREsoft(C, tau);
        else      
            C_thr_new = multiscaleComplexSoft(C, tau, lambda);
        end
        ...
        var_est =  immse(pyramid(C), pyramid(W0)); hist.x_mse(iter);
        for s =1:scales
            tau{s} = struct('H', var_est, 'V', var_est, 'D', var_est);
        end
        tau{scales}.A = var_est;  
    ...
    Nit = sum(hist.timer>0);
    
    hist.x_mse = hist.x_mse(1:Nit);
    hist.timer = hist.timer(1:Nit);
