...
opts.SURE = 1;
opts.saveHist = 1;
opts.denoiserDiv = 2; % 1 for VDAMP-S or 2 for VDAMP-alpha
...
for iter = 1:numel(it_choice)  
    ...
    C = multiscaleDecomp(hist.r1(:,:,ii),opts.scales);
    IC = pyramid(C);
    diff = abs(IC-I0);
...
[re, im] = subband_kurtosis(w0, hist.r1(:,:,end));
